...
namespace SustainableForaging.UI
{
    public class View
    {
        ...
        public DateTime GetKGReportDate()
        {
            DisplayHeader(MainMenuOption.ReportKgPerItem.ToLabel());
            return io.ReadDate("Select a date [MM/dd/yyyy]: ");
        }

        public DateTime GetReportCategoryValueDate()
        {
        ...
        {
            DisplayHeader(MainMenuOption.ViewForagers.ToLabel());
            return io.ReadRequiredString("Enter State: ");
        }

        public int GetForagerViewOption()
        {
            DisplayHeader(MainMenuOption.ViewForagers.ToLabel());
            return io.ReadInt(@"1. View By State
2. View By Name
Select an option by the index: ", 1, 2);
        }
        ...
        public Item MakeItem()
        {
            ...
            item.Name = io.ReadRequiredString("Item Name: ").Replace(",", "");
        ...
        public Forager MakeForager()
        {
            ...
            forager.FirstName = io.ReadRequiredString("First Name: ").Replace(",", "");
            forager.LastName = io.ReadRequiredString("Last Name: ").Replace(",", "");
            forager.State = io.ReadRequiredString("State - Abbreviation Only: ").Replace(",", "");
        ...
        public void DisplayKgReport(Dictionary<Item, decimal> kgOfItems)
        {
            if (kgOfItems == null || kgOfItems.Count == 0)
            {
                io.PrintLine("No items found.");
                return;
            }

            foreach (var item in kgOfItems)
            {
                io.PrintLine(
        ...
        public void DisplayTotalValueByCategoryReport(Dictionary<Category, decimal> totalValueByCat)
        {
            if (totalValueByCat == null || totalValueByCat.Count == 0)
            {
                io.PrintLine("No items found.");
                return;
            }

            foreach (var item in totalValueByCat)
            {
                io.PrintLine(
