...
namespace SustainableForaging.BLL
{
    public class ForageService
    {
        ...
        public Dictionary<Item, decimal> ReportKg(List<Forage> forages)
        {
            /*
            ...
            */
            ...
            Dictionary<Item, decimal> kgOfItems = forages.GroupBy(f => f.Item).ToDictionary(f => f.Key, f => f.Sum(f => f.Kilograms));
            return kgOfItems;
        ...
        private Result<Forage> Validate(Forage forage)
        {
            ...
            List<Forage> currentforages = forageRepository.FindByDate(forage.Date);

            var currForages = currentforages.FirstOrDefault(x => x.Item.Id == forage.Item.Id && x.Forager.Id == forage.Forager.Id);

            if (currForages != null)
            {
                result.AddMessage($"Forage is a duplicate.");
                return result;
            }
